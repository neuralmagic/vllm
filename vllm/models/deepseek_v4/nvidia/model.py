...
class DeepseekV4Model(nn.Module, EagleModelMixin):
    def _maybe_add_hidden_state(
        ...
        # DeepSeek-V4 keeps a multi-stream (hc_mult) residual, so the auxiliary
        # hidden state is the post-`mhc` projection flattened over the stream
        # dimension to match the DFlash/Eagle3 drafter input layout.
        if layer_idx in self.aux_hidden_state_layers:
            if residual is not None and post_mix is not None:
                value = mhc_post_tilelang(hidden_states, residual, post_mix, res_mix)
            else:
                value = hidden_states
