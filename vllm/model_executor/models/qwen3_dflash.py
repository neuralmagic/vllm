...
def _dense_draft_cache_config(cache_config: CacheConfig | None) -> CacheConfig | None:
    """Return a cache config suitable for the dense DFlash draft layers.

    A DeepSeek-V4 target rewrites the shared cache dtype to the MLA-specific
    ``fp8_ds_mla`` packed layout when its sparse-MLA layers are built. The
    DFlash draft uses standard dense attention and cannot consume that layout,
    so fall back to the regular ``fp8`` format for the draft's KV cache.
    """
    if cache_config is None or cache_config.cache_dtype != "fp8_ds_mla":
        return cache_config
...
class DFlashQwen3Model(nn.Module):
    def __init__(
        ...
        *,
        ...
        cache_config = _dense_draft_cache_config(current_vllm_config.cache_config)
