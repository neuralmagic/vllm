...
def _handle_fused_layers(func):
    ...
    def fused_layer_handler(layer_name: Optional[str], module: Module,
        ...
        """
        Wrapper function specifically designed to support the find_matched_target function. 

        It handles cases where the provided layer name corresponds to a fused layer in vllm, 
        mapping it to its equivalent unfused layer name based on the predefined fused_layer_map. 
        If the original layer name raises a ValueError in the wrapped function, this handler 
        will attempt to resolve the issue by substituting the fused layer name.
        ...
        :return: The result of the wrapped find_matched_target function with the resolved layer name.
        :raises ValueError: If the layer name cannot be resolved to a valid target.
        """
        try:
            ...
            if layer_name:
                ...
                unfused_proj_name = fused_layer_map.get(
                    fused_proj_name, fused_proj_name)
