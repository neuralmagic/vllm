...
class PrometheusStatLogger(AggregateStatLoggerBase):
    ...
    def __init__(
        ...
        # Per-(model, layer, rank) routed-token counts. Balancedness is
        # derivable via PromQL, e.g.
        #   avg by (model, layer) (vllm:eplb_tokens_per_rank)
        #   / max by (model, layer) (vllm:eplb_tokens_per_rank)
        ...
        self.gauge_eplb_tokens_per_rank = self._gauge_cls(
            ...
            documentation=(
                ...
                "EPLB sample. Balancedness is derivable via "
                "avg by (...) / max by (...). Only populated when "
