...
class GPUModelRunner(
    ...
    def profile_cudagraph_memory(self) -> int:
        ...
        with self._freeze_gc(), graph_capture(device=self.device):
            ...
            for mode, descs in capture_descs:
                counter_before = compilation_counter.num_cudagraph_captured
                is_full = mode == CUDAGraphMode.FULL
                ...
                for i, desc in enumerate(profile_descs):
                    ...
                    self._warmup_and_capture(
                        ...
                        profile_seq_lens=(
                            ...
                            if is_full and is_first
                            else None
                        ),
                        before_capture_hook=before_capture_hook,
                        num_warmups=None if is_first else 0,
                    )
                    current_platform.synchronize()
                    mem_after = current_platform.mem_get_info()[0]
                    mem_samples.append(mem_before - mem_after)

                first_capture = mem_samples[0]
                measured_per_graph = mem_samples[1] if len(mem_samples) > 1 else 0
                # Use at least 1 MiB per graph to account for driver overhead
                # that may not be captured due to memory alignment
                per_graph = max(measured_per_graph, 1 << 20)
                count = len(descs)

                # Only add first_capture for the first mode since all modes
                # share the same pool and the pool setup cost is paid once
                if is_first_mode:
                    shared_memory_estimate += first_capture
                    per_graph_estimate += per_graph * (count - 1)
                    is_first_mode = False
                else:
                    per_graph_estimate += per_graph * count
                if mode == CUDAGraphMode.PIECEWISE:
                    piecewise_graphs_captured = (
                        compilation_counter.num_cudagraph_captured - counter_before
                    )
    ...
    def _warmup_and_capture(
        ...
        before_capture_hook: Callable[[], None] | None = None,
        ...
        if before_capture_hook is not None:
            before_capture_hook()
        self._dummy_run(
            num_tokens,
            cudagraph_runtime_mode=cudagraph_runtime_mode,
