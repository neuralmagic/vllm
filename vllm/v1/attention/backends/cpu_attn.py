...
class TorchSDPAMetadataBuilderV1(AttentionMetadataBuilder[TorchSDPAMetadata]):
    ...
    def reorder_batch(
        ...
        reorder_prompt_list = self.reorder_prompt_req_index_list[:prompt_list_idx][
            -reorder_req_num:
        ]
        reorder_decode_list = self.reorder_decode_req_index_list[:decode_list_idx][
            :reorder_req_num
        ]
    ...
    def build(
        ...
        max_prefill_seq_len = (
            seq_lens_np[:num_prompt_req].max().item() if num_prompt_req > 0 else 0
        )
        max_decode_seq_len = (
            seq_lens_np[num_prompt_req:num_reqs].max().item()
            if num_prompt_req < num_reqs
            else 0
        )
        ...
        num_decode_tokens = int(
            query_start_loc_cpu[num_reqs].item() - num_prefill_tokens
        )
        ...
        attn_metadata = TorchSDPAMetadata(
            ...
            kv_start_loc=self.seq_start_loc_cpu[: num_prompt_req + 1],  # prefill
            prefill_block_tables=block_table_tensor[:num_prompt_req],  # prefill
