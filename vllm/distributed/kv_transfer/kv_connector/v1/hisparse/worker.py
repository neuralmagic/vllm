...
def _select_exact_row_mirrors(
    ...
    matches = np.searchsorted(starts[order, 0], source_slots, side="right") - 1
    candidate_ids = order[np.maximum(matches, 0)]
    offsets = source_slots - starts[candidate_ids, 0]
...
class HiSparseConnectorWorker:
    ...
    def initialize(
        ...
        if self._resident_group_id is None:
    ...
    def _require_row_mirrors(self, num_rows: int) -> tuple[SparseKVRowMirror, ...]:
        ...
        from_resident = self._row_mirrors_from_resident
    ...
    def _enqueue_post_forward_transfers(self) -> None:
