...
def wait_for_gpu_memory_to_clear(*,
    ...
    while True:
        ...
        for k, (used, total) in output.items():
            print(f'{k}={used}/{total}; ', end='')
        print('')

        is_free = lambda used, total: used <= (
            threshold_bytes / 2**30
        ) if threshold_bytes is not None else used / total <= threshold_ratio
        threshold = f"{threshold_bytes/2**30} GiB" \
            if threshold_bytes is not None else threshold_ratio
