...
from vllm.model_executor.layers.fused_moe.cutlass_moe import (
    CutlassExpertsBlockedFp8, CutlassExpertsFp8)
from vllm.model_executor.layers.fused_moe.fused_moe import fused_topk
from vllm.model_executor.layers.fused_moe.modular_kernel import (
    FusedMoEModularKernel)
from vllm.platforms import current_platform
from vllm.model_executor.layers.quantization.utils.fp8_utils import (
    per_block_cast_to_fp8, scaled_dequantize)
...
def pplx_cutlass_blocked_moe(
    ...
    prepare_finalize = PplxPrepareAndFinalize(
        ...
        dp_size,
        quant_dtype=torch.float8_e4m3fn,
        per_act_token=per_act_block,
        block_shape=(128, 128) if per_act_block else None,
    )

    print("PER ACT BLOCK FROM TEST:", per_act_block)

    experts = CutlassExpertsBlockedFp8((num_experts + world_size - 1) // world_size,
                                out_dtype, per_act_block=per_act_block, use_batched_format=True)
    ...
    # print("chunk a:", a_chunk)
    # print("chunk w1:", chunk_by_rank(w1, rank, world_size))
    # print("chunk w2:", chunk_by_rank(w2, rank, world_size))
    # print("chunk w1_scale:", chunk_by_rank(w1_scale, rank, world_size))
    # print("chunk w2_scale:", chunk_by_rank(w2_scale, rank, world_size))
    # print("chunk a1_scale:", (chunk_by_rank(a1_scale, rank, world_size)
    #     if per_act_block else a1_scale[rank]))
    
    # return torch.zeros((rank_num_tokens, hidden_dim), device=device, dtype=a.dtype)
...
def _pplx_blocked_moe(
    ...
    with set_current_vllm_config(vllm_config):
        ...
        pplx_output = pplx_cutlass_blocked_moe(pgi, dp_size, a, w1, w2, w1_scale,
                                       w2_scale, topk_weights, topk_ids,
                                       a1_scale, out_dtype, per_act_block,
                                       block_size)
...
@pytest.mark.parametrize("e", NUM_EXPERTS)
@pytest.mark.parametrize("topk", TOP_KS)
@pytest.mark.parametrize("per_act_block", [True, False]) #[True, False])
@pytest.mark.parametrize("world_dp_size", [[2, 1]])  #, [4, 2]])
# @pytest.mark.parametrize("m", [6])
# @pytest.mark.parametrize("n", [3072])
# @pytest.mark.parametrize("k", [1536])
...
def test_blocked_cutlass_moe_pplx(
    ...
    with set_current_vllm_config(vllm_config):
        ...
        print("a1_scale:", a_scale1, a_scale1.shape)
        print("w1_d:", w1_d)
        print("w2_d:", w2_d)

        parallel_launch(world_size, _pplx_blocked_moe, dp_size, a, w1_q, w2_q,
                        w1_scale, w2_scale, topk_weights, topk_ids, a_scale1,
                        dtype, a, w1_d, w2_d, per_act_block, 128)
