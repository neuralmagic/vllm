...
import vllm.plugins
from vllm.compilation.fusion import (FUSED_OPS, FusedRMSQuantKey,
...
class TestModel(torch.nn.Module):
    ...
    def __init__(self, hidden_size: int, eps: float, static: bool,
        ...
        self.quant_key = QuantKey(dtype=FP8_DTYPE,
                                  scale=quant_scale,
                                  symmetric=True)
        if static:
    ...
    def ops_in_model_before(self):
        return [QUANT_OPS[self.quant_key]] if self.enable_quant_fp8 \
                else [torch.ops.aten.reciprocal]

    def ops_in_model_before_partial(self):
        return [RMS_OP, RMS_ADD_OP] if self.enable_rms_norm \
...
def test_fusion_rmsnorm_quant(dtype, hidden_size, num_tokens, eps, static,
    ...
    with vllm.config.set_current_vllm_config(vllm_config):
        ...
        backend.check_before_ops(model.ops_in_model_before_partial(),
        ...
        if not enable_rms_norm:
            n_add_nodes = lambda g: \
                sum(1 for _ in find_op_nodes(torch.ops.aten.add, g))
