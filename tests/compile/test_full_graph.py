...
INDUCTOR_GRAPH_PARTITION = [False, True] if (
    is_torch_equal_or_newer("2.9.0.dev")) else [False]


@pytest.mark.parametrize("custom_ops", ["+quant_fp8", "-quant_fp8"])
@pytest.mark.parametrize("inductor_graph_partition", INDUCTOR_GRAPH_PARTITION)
def test_default_fusion(custom_ops: str, inductor_graph_partition: bool,
                        caplog_vllm, monkeypatch):
...
@pytest.mark.parametrize("custom_ops", ["+quant_fp8", "-quant_fp8"])
@pytest.mark.parametrize("inductor_graph_partition", INDUCTOR_GRAPH_PARTITION)
def test_default_fusion_tp2(custom_ops: str, inductor_graph_partition: bool,
                            caplog_vllm, monkeypatch):
    model = "nvidia/Llama-4-Scout-17B-16E-Instruct-FP8"
    model_kwargs = {"kv_cache_dtype": "fp8", "max_model_len": 1024}
    backend = _Backend.FLASHINFER
