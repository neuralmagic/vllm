...
def test_shutdown_discards_pending_tasks(fs_tier):
    ...
    assert len(tier._pool._scheduler._load_q) == 0
    assert len(tier._pool._scheduler._store_q) == 0
...
def test_wait_idle_blocks_until_tasks_complete():
    ...
    pool = DualQueueThreadPool(n_read_threads=1, n_write_threads=1, block_size=1)
    pool.enqueue_store(
        job_id=1,
        n_tasks=1,
        tasks=[task],
...
def test_store_job_parallelized_across_threads():
    ...
    pool = DualQueueThreadPool(
        n_read_threads=0, n_write_threads=n_threads, block_size=1
    )
    try:
