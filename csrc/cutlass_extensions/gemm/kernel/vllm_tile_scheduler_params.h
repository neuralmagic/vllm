...
struct PersistentTileSchedulerSm90Params32bit {
  ...
  initialize(
    ...
    if (raster_order == RasterOrder::AlongN) {
      ...
      divmod_cluster_blk_major_ = FastDivmod(problem_blocks_n / cluster_shape.n());
    }
    else {
      ...
      divmod_cluster_blk_major_ = FastDivmod(problem_blocks_m / cluster_shape.m());
    }
  }
...
template<bool enable_seperate_reduction>
struct PersistentTileSchedulerSm90StreamKParams32bit {
  ...
  using UnderlyingParams = PersistentTileSchedulerSm90Params32bit;
  ...
  FastDivmod divmod_clusters_mnl_{};
  ...
  FastDivmod divmod_sk_groups_{};

  // Number of stream-K units in each group
  FastDivmod divmod_sk_units_per_group_{};
  ...
  FastDivmod divmod_epilogue_subtile_{};
  ...
  CUTLASS_HOST_DEVICE
  int32_t
  div_cluster_size(int32_t dividend) const {
  ...
  initialize(
    ...
    divmod_sk_groups_ = FastDivmod(static_cast<uint64_t>(groups));
    divmod_sk_units_per_group_ = FastDivmod(static_cast<uint64_t>(sk_units / groups));
    ...
    divmod_clusters_mnl_ = FastDivmod(sk_units / cluster_size);
    ...
    divmod_epilogue_subtile_ = FastDivmod(epilogue_subtile);
  ...
  get_num_sk_tiles(
    ...
    if (decomposition_mode == DecompositionMode::Heuristic) {
      ...
      if (full_waves > 0 && 2 * tail_tiles >= ctas_per_wave) {
        return 0;
      }
    }
  ...
  should_perform_separate_reduction(uint32_t epilogue_subtile, uint64_t sk_units, uint64_t sk_tiles, uint64_t dp_tiles, uint64_t ctas_per_wave) {
    ...
    if constexpr (enable_seperate_reduction) {
  ...
  set_params_basic(
    ...
    divmod_cluster_shape_major_ = underlying_params.divmod_cluster_shape_major_;
    divmod_cluster_shape_minor_ = underlying_params.divmod_cluster_shape_minor_;
    divmod_batch_ = FastDivmod(blocks_m * blocks_n);
    divmod_tiles_per_output_tile_ = FastDivmod(k_tiles_per_output_tile);
    divmod_sk_groups_ = FastDivmod(1u);
    ...
    divmod_clusters_mnl_ = FastDivmod((blocks_m * blocks_n * blocks_l) / cluster_size);
    ...
    sk_tiles_ = 0;
    sk_units_ = 0;
    divmod_sk_units_per_group_ = FastDivmod(1u);
...
struct vLLMPersistentTileSchedulerSm90StreamKParams
    : PersistentTileSchedulerSm90StreamKParams32bit<false> {
  using UnderlyingParams = PersistentTileSchedulerSm90StreamKParams32bit<false>;
