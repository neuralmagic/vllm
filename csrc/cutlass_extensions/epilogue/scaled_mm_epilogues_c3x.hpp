...
#include "cutlass_extensions/epilogue/broadcast_ptr_array_load_epilogue_c3x.hpp"
...
struct ScaledEpilogueBase {
 protected:
  ...
  template <typename T>
  using ColOrScalarLoadPtrArray = cutlass::epilogue::fusion::Sm90ColOrScalarBroadcastPtrArray<
      0 /*Stages*/, typename EpilogueDescriptor::TileShape, T,
      Stride<Int<1>, Int<0>, Int<0>>>;

  template <typename T>
  using RowOrScalarLoadPtrArray = cutlass::epilogue::fusion::Sm90RowOrScalarBroadcastPtrArray<
      0 /*Stages*/, typename EpilogueDescriptor::TileShape, T,
      Stride<Int<0>, Int<1>, Int<0>>>;
  ...
  template <typename Descriptor, typename T>
  static auto args_from_tensor(const T* const* data_ptr) {
    using Arguments = typename Descriptor::Arguments;
    static_assert(std::is_same_v<Descriptor, ColOrScalarLoadPtrArray<T>> ||
                  std::is_same_v<Descriptor, RowOrScalarLoadPtrArray<T>>);
    return Arguments{data_ptr};
  }
};
...
*/
template <typename ElementAcc, typename ElementD, typename EpilogueDescriptor>
struct ScaledEpiloguePtrArray
    : private ScaledEpilogueBase<ElementAcc, ElementD, EpilogueDescriptor> {
 private:
  using SUPER = ScaledEpilogueBase<ElementAcc, ElementD, EpilogueDescriptor>;
  using Accum = typename SUPER::Accum;
  using ScaleA = typename SUPER::template ColOrScalarLoadPtrArray<float>;
  using ScaleB = typename SUPER::template RowOrScalarLoadPtrArray<float>;
...
//   }
  ...
  static ArgumentType prepare_args(const float* const* a_scales_ptr,
    ...
    auto a_args = SUPER::template args_from_tensor<ScaleA, float>(a_scales_ptr);
    auto b_args = SUPER::template args_from_tensor<ScaleB, float>(b_scales_ptr);
